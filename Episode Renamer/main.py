...
# ---------- Compatibility shims (icons/colors across Flet versions) ----------
ICONS = getattr(ft, "icons", None)
if ICONS is None:
    # Older Flet exposes PascalCase enums
    ICONS = ft.Icons  # type: ignore[attr-defined]

COLORS = getattr(ft, "colors", None)
if COLORS is None:
    COLORS = ft.Colors  # type: ignore[attr-defined]
...
def main(page: ft.Page):
    ...
    page.title = "Jellyfin Episode Renamer (Flet)"
    ...
    # ---------- Controls ----------
    title_text = ft.Text("Jellyfin Episode Renamer", size=22, weight=ft.FontWeight.BOLD)
    ...
    pick_btn = ft.ElevatedButton("Pick Folder", icon=ICONS.FOLDER_OPEN)
    ...
    preview_btn = ft.FilledButton("Preview", icon=ICONS.PREVIEW)
    apply_btn = ft.ElevatedButton("Apply Renames", icon=ICONS.DRIVE_FILE_RENAME_OUTLINE, disabled=True)
    undo_btn = ft.OutlinedButton("Undo Last Run (Ctrl+Z)", icon=ICONS.UNDO, disabled=(settings.get("last_log", "") == ""))
    status_text = ft.Text("", selectable=True)

    progress_bar = ft.ProgressBar(value=0, width=400, visible=False)
    cancel_btn = ft.TextButton("Cancel", icon=ICONS.CANCEL, visible=False)
    ...
    def add_row(idx: int, src: Path, dst: Path, status: str, conflict: bool = False, skip: bool = False):
        ...
        color = COLORS.RED if conflict else (COLORS.GREY if skip else None)
        row = ft.DataRow(
            cells=[
    ...
    # ---------- PubSub for thread-safe UI updates ----------
    if not hasattr(page, "pubsub"):
        # Very old Flet? Politely raise to guide upgrade.
        raise RuntimeError(
            "Your Flet build doesn't provide `page.pubsub`. "
            "Please upgrade:  pip install -U flet"
        )

    def on_pubsub(msg: dict):
        t = msg.get("type")
        if t == "progress":
            ...
            done, total, errors, was_canceled = msg["done"], msg["total"], msg["errors"], msg["canceled"]
            progress_bar.visible = False
            cancel_btn.visible = False
            preview_btn.disabled = False
            pick_btn.disabled = False
            apply_btn.disabled = False
            msg_text = f"Done. Renamed {done}/{total} file(s)."
            if was_canceled:
                msg_text = f"Canceled. Renamed {done}/{total} file(s) before cancel."
            if errors:
                msg_text += "\nErrors:\n" + "\n".join(f"- {x}" for x in errors)
            status_text.value = msg_text
            status_text.update()
            do_preview(None)  # refresh
            ...
            with applying_lock:
                try:
                    for src, dst in ops:
                        ...
                        page.pubsub.send_all({"type": "progress", "done": done, "total": total})
                    ...
                    if changes:
                        try:
                            ...
                            save_settings(settings)
                            page.pubsub.send_all({"type": "enable_undo"})
                        except Exception as ex:
    ...
    page.add(
        ft.Column(
            [
                ...
                ft.Divider(),
                status_text,
                ft.Text("Shortcuts:  Ctrl+P Preview   •   Ctrl+Enter Apply   •   Ctrl+Z Undo", size=12, color=COLORS.GREY),
            ],
            spacing=12,
        )
    )
